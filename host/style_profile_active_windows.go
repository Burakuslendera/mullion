...
//
// CaptionSysMenuNCCalc keeps WS_CAPTION, preserving shell support for Snap and
// the minimise/restore animations, and adds WS_SYSMENU, which is what makes a
// right-click on the app-region caption open the standard system menu.
// WM_NCCALCSIZE then extends the client area over the frame, so the WebView
// covers the whole window and the title bar is HTML.
//
