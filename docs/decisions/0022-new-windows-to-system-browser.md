...
**Status:** Accepted. The launch moved off the UI thread by [0029](./0029-system-browser-launch-off-the-ui-thread.md); the routing choice is unchanged. [0043](./0043-external-routes-are-uri-only-os-activations.md) records the historical URI-only OS-activation, concurrency, gesture, request, and browser-session rationale. Current implementation: [Issue #116 bridge disposition](../bridge.md#issue-116-current-disposition).
