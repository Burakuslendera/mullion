...
**Instead.** No port. Assets are served from an embedded filesystem through WebView2's `WebResourceRequested` interception: navigation targets a virtual host name, the request is answered in-process, nothing binds to the network stack. Enforced statically — a grep for `net.Listen`, `http.Server`, `127.0.0.1:0` and `localhost` across the tree is part of the verification checklist and must return nothing, with one exemption: the default virtual host name, and only where it stands alone (decisions/0030).
